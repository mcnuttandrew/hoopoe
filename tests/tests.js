...
tape("#getFile non-extant file and then #writeFile to make it exists", (t) => {
  let msg = "";
  const expectedError =
    '{"errno":-2,"code":"ENOENT","syscall":"open","path":"./tests/fake-file.json"}';
    ...
    return getFile("./tests/fake-file.json")
      ...
      .then(() =>
        t.deepEqual(
          msg,
          expectedError,
          "should find that an error was thrown as expected"
        )
      );
  };
  Promise.resolve()
    ...
    .then((foundFile) =>
      t.deepEqual(
        foundFile,
        "Hi mom",
        "should find the fake file correctly formed"
      )
    )
...
tape("#executeCommandLineCmd & #executePromisesInSeries", (t) => {
  Promise.all(
    [
      () =>
        executeCommandLineCmd("ls tests").then(({ stdout, stderr }) => {
          t.deepEqual(
            stdout,
            "test-file.json\ntests.js\n",
            "should find a out"
      ...
      () =>
        executePromisesInSeries([
          ...
          () =>
            executeCommandLineCmd("ls test-folder").then(({ stdout }) => {
              t.deepEqual(
                stdout,
                "test-file.json\n",
          ...
          () =>
            executeCommandLineCmd("rm -rf test-folder").then(({ stdout }) => {
        ...
        return executeCommandLineCmd("smooshsmashsmash")
          ...
          .then(() =>
            t.equal(
              ...
              msg,
              "should find correct error message"
            )
          );
      },
...
tape("#getFileNamesFromDir", (t) => {
  getFileNamesFromDir("tests")
    .then((result) => {
      t.deepEqual(
        result,
        ["test-file.json", "tests.js"],
        "should return the correct set of file names"
      );
    })
    .then(() => t.end());
});
...
tape("#sleep pass through", (t) => {
  Promise.resolve()
    ...
    .then(sleep(30))
    .then((x) =>
      t.deepEqual(x, [1, 2, 3], "should find the pass through worked correctly")
    )
    .then(() => t.end());
});
  ...
  Promise.all([
    ...
    .then(() => {
      t.deepEqual(
        order,
        ["B", "A", "C"],
        "should find the order of sleeps is correct"
      );
    })
    .then(() => t.end());
});
